
import 'package:flutter/material.dart';

const Color colorTheme1 = Color(0xff58c2c4);
const Color colorTheme2 = Color(0xff2367f6);
const Color colorTheme3 = Color(0xff022c41);
const Color colorTheme4 = Color(0xffd24129);
const Color colorTheme5 = Color(0xff1f7775);
const Color colorNull = Colors.grey;

const String routeTechnician = "ROUTE_TECHNICIAN";
const String routeEngineer = "ROUTE_ENGINEER";
const String routeTechnicianDetail = "ROUTE_TECHNICIAN_DETAIL";
const String routeInventory = "ROUTE_INVENTORY";
const String routeCheckIn = "ROUTE_CHECKIN";
const String routeCheckOut = "ROUTE_CHECKOUT";
const String routeMaterialInfo = "ROUTE_MATERIAL_INFO";
const String routeRegisterItem = "ROUTE_REGISTER_ITEM";
const String routePurchaseRequest = "ROUTE_PURCHASE_REQUEST";
const String routeMateralRequest = "ROUTE__MATERIAL_REQUEST";
const String routeMaterialRequestView = "ROUTE__MATERIAL_REQUEST_VIEW";
const String routeStockRequest = "ROUTE_STOCK_REQUEST";
const String routePurchaseOrder = "ROUTE_PURCHASE_ORDER";
const String routeProcurement = "ROUTE_PROCUREMENT";
const String routeDashboard = "ROUTE_DASHBOARD";
const String routeDetails = "ROUTE_DETAILS";
const String routeCheckInInfo = "ROUTE_CHECKIN_DETAILS";
const String routeStockIn = "ROUTE_STOCKIN";
const String routeAddStockIn = "ROUTE_ADD_STOCKIN";
const String routeMaterial = "ROUTE_MATERIAL";
const String routeUtilities = "ROUTE_UTILITIES";
const String routeSignature = "ROUTE_SIGNATURE";
const String routeMaterialCheckinRequest = "ROUTE_CHECKIN_REQUEST";
const String routeLeaderboard = "ROUTE_LEADERBOARD";
const String routeAttendance = "ROUTE_ATTENDANCE";
const String routeRoutineInspection = "ROUTE_ROUTINE_INSPECTION";

// NETWORK - GET
const String getDashboard = "";
const String getCheckout = "";
const String getCheckoutDetails = "";
const String getCheckin = "";
const String getNewCheckin = "";
const String getNewCheckinInfo = "";
const String getStock = "";
const String getItems = "";
const String getItemDetail = "";
const String getThreshold = "";
const String getStatuses = "";
const String getTask = "";
const String getTaskDetail = "";

// NETWORK - POST
const String postDOImages = "";
const String postItemDetail = "";
const String postTaskStatus = "";

// PREFERENCES - KEY
const String kName = "NAME";
const String kToken = "TOKEN";
const String kID = "ID";
const String kUsername = "USERNAME";
const String kRole = "ROLE";
