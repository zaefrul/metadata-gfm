...
=======
# GEMS Mobile (Flutter) – Copilot Field Manual

## Stack & Runtime
- Flutter mobile app for iOS/Android; companion to GEMS2 PHP backend
- Run `flutter pub get` to install dependencies
- Use `flutter run` for development; `flutter build apk|ios` for releases
- Firebase configured via `lib/firebase_config.dart` and `google-services.json` / `GoogleService-Info.plist`

## Project Structure
```
lib/
├── main.dart           # App entry, Firebase init, routes
├── controller/         # Feature modules (screens + logic)
├── model/              # Data classes with built_value serialization
...
Each feature lives in `lib/controller/<Feature>/`:
- `WorkOrder/` – Work order listing and complaint submission
- `PPM/` – Preventive maintenance tasks, inspections
- `Storekeeper/` – Material requests, inventory management
- `Attendance/` – Staff attendance tracking
- `Homepage/` – Dashboard, signature, support

### Models (`lib/model/`)
- Use `built_value` with `.g.dart` generated files
- Run `flutter pub run build_runner build` after model changes
- Key models: `User`, `Complaint`, `WorkOrder`, `Task`, `Material`

### Network Layer (`lib/utils/network.dart`)
```dart
final String netDomain = "https://gems.metadatasystem.my";
// Provider class handles auth headers and device ID
Provider(fetchURL: '/api/endpoint').fetch();
```
- All requests include JWT token via `Authorization: Bearer {token}`
- Device ID attached via `Deviceid` header
- Response parsed through `ResponseValue` model

## Authentication
- Login via `login(username, password)` in `network.dart`
- User session stored in SharedPreferences (`User.getPrefUser`)
- Token refreshed on app restart; no auto-refresh logic

## API Integration
Backend APIs live at `https://gems.metadatasystem.my/api/`:
- `/m_login.php` – Authentication
- `/m_wo.php` – Work orders
- `/m_ppm.php` – PPM tasks
- `/m_inventory.php` – Materials/inventory

## Theming (`lib/utils/reference.dart`)
```dart
AppColors.primary      // #276EF1
AppColors.success      // #28A745
AppColors.warning      // #FFC107
AppColors.danger       // #DC3545
```
Use `getButtonBgColorByStatus(status)` for consistent status coloring.

## Push Notifications
- Firebase Cloud Messaging (FCM) configured in `main.dart`
- Background handler: `_firebaseMessagingBackgroundHandler`
- Local notifications via `flutter_local_notifications`
- APNS token fetched on iOS startup
...
| `built_value` | Immutable models |
| `device_info_plus` | Device identification |
| `month_year_picker` | Date selection UI |

## Development Notes
- `MyHttpOverrides` bypasses SSL certificate validation (dev only)
- Debug prints use `debugPrint()` for Flutter-safe logging
- Navigation via `GlobalKey<NavigatorState> navigatorKey`
>>>>>>> Stashed changes
